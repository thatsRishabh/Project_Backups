...
<body>
    ...
    <!-- database delete querry here -->
    <?php

    if(isset($_GET['delete'])){
        $sno = $_GET['delete'];
        $sql = "DELETE FROM `contact` WHERE `sno` = $sno";
        $result = mysqli_query($conn, $sql);
        echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
            <strong>Success!!</strong> Your contact has been deleted! 
    ...
    if($method=='POST'){
        ...
        $existSql = "select * from `contact` where phone_no = '$number'";
        ...
        if($numRows>0 || $number ==0){
            ...
            $sql = "INSERT INTO `contact` (`sno`, `name`, `phone_no`, `email`, `dt`) VALUES (NULL, '$name', '$number', '$email', current_timestamp());";
            $result = mysqli_query($conn, $sql);
            echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
    ...
    <div class="container my-3 mx-auto" style="width: 50%;">
        <form action="/crud1/index.php" method="post">
            <div class="mb-3">
 ...
 <hr>
    ...
    <div class="card" style="width: 40rem;">
        ...
        <ul class="list-group list-group-flush">
            ...
            <li class="list-group-item">'.$row['phone_no'].'</li>
            <li class="list-group-item">'.$row['email'].'</li>
        </ul>  
    </div>
    <button type="button" class="delete btn btn-primary my-2" id='.$row['sno'].'>Delete</button>
    </div>';
    }
    ?>
    ...
    Array.from(deletes).forEach((element) => {
        element.addEventListener("click", (e) => {
            ...
            if (confirm("Are you sure you want to delete this note!")) {
                console.log("yes");
                window.location = `/crud1/index.php?delete=${sno}`;
